...
const title=$('title');
tabs.forEach(b=>b.onclick=()=>{tabs.forEach(x=>x.classList.remove('active'));b.classList.add('active');panels.forEach(p=>p.classList.toggle('hidden',p.id!==b.dataset.tab));title.textContent=b.textContent.replace(/^[^A-Za-z✦◈▣⌂⌕◫✎⚙＋]+/,'').trim();if(b.dataset.tab==='chats'||b.dataset.tab==='servers')discover()});
...
let discovered=[];
async function discover(){try{const [tab]=await chrome.tabs.query({active:true,currentWindow:true});if(!tab?.id)return;const data=await chrome.tabs.sendMessage(tab.id,{type:'discover'});discovered=data?.channels||[];renderDiscovery()}catch(e){discovered=[];renderDiscovery()}}
function renderDiscovery(){const q=($('chatFilter')?.value||'').toLowerCase();const arr=discovered.filter(x=>x.name.toLowerCase().includes(q));$('chatCount').textContent=discovered.length;$('aChats').textContent=discovered.length;$('serverCount').textContent=new Set(discovered.map(x=>x.server).filter(Boolean)).size;$('aServers').textContent=new Set(discovered.map(x=>x.server).filter(Boolean)).size;$('chatList').innerHTML=arr.length?arr.map(x=>`<div>▣ ${x.name}<br><small>${x.server||'Discord'}</small></div>`).join(''):'<div class="empty">Keine sichtbaren Channels gefunden.</div>';$('serverList').innerHTML=[...new Set(discovered.map(x=>x.server).filter(Boolean))].map(x=>`<div>⌂ ${x}</div>`).join('')||'<div class="empty">Keine Server erkannt.</div>'}
$('chatFilter').oninput=renderDiscovery;
$('searchBox').oninput=e=>{const q=e.target.value.toLowerCase();$('results').innerHTML=q?discovered.filter(x=>x.name.toLowerCase().includes(q)).map(x=>`<div class="settingCard">▣ ${x.name}<br><small>${x.server||'Discord'}</small></div>`).join('')||'Keine Treffer.':'Noch keine Suche.'};
